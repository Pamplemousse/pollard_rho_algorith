...
void test_returns_an_error_code_if_given_ai_is_null(void) {
  ...
  int code = f(&xi, NULL, &bi, &g, &h, &modulus, &order, NULL);
  ASSERT (code == -1);
}
...
void test_returns_an_error_code_if_given_bi_is_null(void) {
  ...
  int code = f(&xi, &ai, NULL, &g, &h, &modulus, &order, NULL);
  ASSERT (code == -1);
}
...
void test_returns_an_error_code_if_given_g_is_null(void) {
  ...
  int code = f(&xi, &ai, &bi, NULL, &h, &modulus, &order, NULL);
  ASSERT (code == -1);
}
...
void test_returns_an_error_code_if_given_h_is_null(void) {
  ...
  int code = f(&xi, &ai, &bi, &g, NULL, &modulus, &order, NULL);
  ASSERT (code == -1);
}
...
void test_returns_an_error_code_if_given_modulus_is_null(void) {
  ...
  int code = f(&xi, &ai, &bi, &g, &h, NULL, &order, NULL);
  ASSERT (code == -1);
}
...
void test_returns_an_error_code_if_given_order_is_null(void) {
  ...
  int code = f(&xi, &ai, &bi, &g, &h, &modulus, NULL, NULL);
  ASSERT (code == -1);
}
...
void test_handbook_values(void) {
  ...
  for (size_t i = 0; i < 14; i++) {
    ...
    if (i > 0) {
      ...
      f(&xi, &ai, &bi, &g, &h, &modulus, &order, NULL);
